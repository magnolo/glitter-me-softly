...
import { EVENT_DATE_ISO, renderBroadcastEmail } from '$lib/server/broadcast-email';
...
export const load: PageServerLoad = async ({ cookies }) => {
	...
	const preview = renderBroadcastEmail({ name: 'Your Guest', now: previewNow });
...
export const actions: Actions = {
	testSend: async ({ request, cookies }) => {
		...
		const { subject, html } = renderBroadcastEmail({ name: 'Test', now: new Date() });
	...
	sendAll: async ({ cookies }) => {
		...
		for (let i = 0; i < recipients.length; i += BATCH_SIZE) {
			...
			const payload = chunk.map((r) => {
				const { subject, html } = renderBroadcastEmail({ name: r.name, now });
				return {
					from: FROM_ADDRESS,
					to: r.email,
					subject,
